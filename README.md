...
- **Add these rules to your `proguard-rules.pro` file:**
  ...
  -keep class com.behaviosec.android.AccelerometerTouchTrackerIosSdk.** { *; }
  ```
...
2. **Generate HTML Documentation with Dokka:**  
   ...
   ```
   ./gradlew :AccelerometerTouchTrackerIosSdk:dokkaGeneratePublicationHtml
   ```
...
1. **Generate the documentation:**
   ...
   ./gradlew :AccelerometerTouchTrackerIosSdk:dokkaGeneratePublicationHtml
   ```
   The output will be in `AccelerometerTouchTrackerIosSdk/build/dokkaDir`.
...
3. **Copy the generated documentation:**
   ```bash
   cp -r AccelerometerTouchTrackerIosSdk/build/dokkaDir/* .
   ```
